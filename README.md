...
4. Многое сломалось из-за пункта 3. В основном потому, что нужно реализовать функцию getNews(skip, top) через метод GET, чем я займусь в ближайшее время.

Задание 7+ (Пятница 12, 17.10)
=======================================
1. Добавление новостей теперь не удаляет весь массив из n элементов, загружая затем массив из n+1 элементов ( 2n+1 операций = O(n)). Теперь оно происходит за константу времени = O(1).
