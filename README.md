...
2. Кнопки удаления и редактирования новости теперь фиксированного размера в vw (при скроллинге не увеличиваются).
...
6. Теперь стоит ограничение на количество вводимых символов в авторизации, регистрации и настройках(везде кроме ссылок по 16 включительно). Эта функция берёт часть работы валидации на себя, а так же не позволяет вводить пользователям в поля сколько угодно символов.
7. Код становится проще и доступнее, делаются блоки кода с пометками, в которых переменные упорядовиваются по алфамиту.
